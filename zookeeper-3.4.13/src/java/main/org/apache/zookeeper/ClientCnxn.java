...
public class ClientCnxn {
     ...
     * @param hostProvider
     *                the list of ZooKeeper servers to connect to 要连接的ZooKeeper服务器列表
     * @param sessionTimeout
     *                the timeout for connections.  连接的超时。
     * @param zooKeeper
     *                the zookeeper object that this connection is related to.  此连接与之关联的zookeeper对象。
     * @param watcher watcher for this connection
     * @param clientCnxnSocket
     *                the socket implementation used (e.g. NIO/Netty)  使用的套接字实现（例如NIO / Netty）
     * @param canBeReadOnly
     *                whether the connection is allowed to go to read-only  在分区的情况下是否允许连接进入只读模式
            ...
            while (state.isAlive()) {
                try {
                    if (!clientCnxnSocket.isConnected()) {
                        ...
                        // don't re-establish connection if we are closing  如果关闭，不要重新连接
